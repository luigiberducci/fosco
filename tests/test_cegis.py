...
class TestCEGIS(unittest.TestCase):
    ...
    def _get_single_integrator_config(self):
        ...
        dn = DomainNames
        domains = {
            name: domain for name, domain in zip([dn.XD.value, dn.UD.value, dn.XI.value, dn.XU.value],
                                                 [XD, UD, XI, XU])
        }

        data_gen = {
            dn.XD.value: lambda n: torch.concatenate([XD.generate_data(n), UD.generate_data(n)], dim=1),
